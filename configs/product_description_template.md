...
**规则 6 — 标题字数强制不超过 100 字符**
生成标题后必须立即计算字符数（含空格）。若超过 100 字符，必须删减次要关键词直至符合限制，不得以任何理由超出。
删减优先级（从低到高保留）：
1. 先删介词短语（如 "for Glass Bead"、"with"）
2. 再删形容词修饰词（如 "Strong"、"Extra"）
3. 最后保留：品牌名 + 核心产品词 + 最关键的 1–2 个差异化特性

**规则 7 — Package Includes 必须逐件列出所有独立配件**
不得仅写产品主体，必须列出包装内所有独立物件。判断方法：
- 查看 1688 详情图中是否有"套装内含"、"包含"、"赠送"等说明
- 查看产品名称中是否含有"套装"、"组合"、"含磁铁球"等词
- 若产品名称含 "w/ball"、"with ball"、"含球"，则包装内必有独立磁铁球，须单独列出
- 若无法从图文确认，必须在交互确认环节向用户询问包装内容物，不得自行猜测
